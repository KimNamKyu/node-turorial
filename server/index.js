...
app.get('/api/board/:board_id', (req, res) => {
    ...
    Board.findOne({ _id: req.params.board_id }, (err, board) => {
        ...
        if (!board) return res.status.apply(404).json({ err: err })
        else res.json(board)
    })
})
...
app.post('/api/board/write', (req, res) => {
    ...
    board.reg_date = new Date(req.body.reg_date);
...
//게시판 수정
app.put('/api/board/modify/:board_id', (req, res) => {
    // Board.findById
    Board.update({ _id: req.params.board_id }, { $set: req.body }, (err, output) => {
...
//게시판 삭제
app.delete('/api/board/:board_id', (req, res) => {
